...
let tooltip1 = "A bigger dice combination will win. If a player rolls out a double then the game becomes a double game, where you must roll only doubles.";
let tooltip2 = "A smaller dice combination will win. If a player rolls out a double then the game becomes a double game, where you must roll only doubles.";
let tooltip3 = "3:3; 5:5; 6:6; 6:5; are winning dices. 1:1; 2:2; 2:1; 4:4; are losing dices. If none is hit, biggest wins";
let tooltip4 = "Overview of the game, turn displays who picks the rule, roll in paranthesis displays who rolls right now, rule is the picked rule and roll is the amount rolled. ";
let tooltip5 = "It’s the best out of 3 games of Clean.";
let tooltip6 = "The first one that rolls 7 wins.";
...
class Play extends React.Component {
    ...
    checkValue = val => {
        if(val === undefined) {}
            ...
            if(!this.state.loading) {
                if(val.target.value > this.state.balance || val.target.value < 0 || !Number.isInteger(parseFloat(val.target.value))) {
                    if(!Number.isInteger(parseFloat(val.target.value)) || val.target.value < 0) {
                        this.setState({value: 0});
                        this.setState({notEnough: "Whole numbers only"});
                    } 
                    if(val.target.value > this.state.balance) {
                        this.setState({value: 0});
    ...
    wonConclusion = () => {
        ...
        setTimeout( () => {
            if(this.state.turn === "Your Turn") {
                this.setState({conclusion: "You are free to pick a rule and play."});
            } else {
                this.setState({conclusion: "Your opponent will pick a rule."});
            }
        }, 2000)
        ...
        return (
            <div id={"play"}>
                ...
                <FormGroup id="firstGroup">
                    <Tooltip title={tooltip1} placement={"right"} TransitionComponent={Zoom}>
                        <FormControlLabel
                            control={
                                <Radio
                    ...
                    <Tooltip title={tooltip2} placement={"right"} TransitionComponent={Zoom}>
                        <FormControlLabel
                            control={
                                <Radio
                    ...
                    <Tooltip title={tooltip3} placement={"right"} TransitionComponent={Zoom}>
                        <FormControlLabel
                            control={
                                <Radio
                ...
                <Tooltip title={tooltip4} placement={"left"} TransitionComponent={Zoom}>
                    <div id="AI">
                ...
                <Tooltip title={tooltip5} placement={"left"} TransitionComponent={Zoom}>
                    <FormControlLabel
                        control={
                            <Radio
                ...
                <Tooltip title={tooltip6} placement={"left"} TransitionComponent={Zoom}>
                    <FormControlLabel
                        control={
                            <Radio
